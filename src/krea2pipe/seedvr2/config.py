...
def load_config(path: str, argv: List[str] = None) -> Union[DictConfig, ListConfig]:
    ...
    # ``__inherit__`` paths are resolved relative to the including file so the
    # configs can live inside the package.
    base_dir = os.path.dirname(os.path.abspath(path))
    config = resolve_recursive(config, partial(resolve_inheritance, base_dir=base_dir))
    return config
...
def resolve_inheritance(config: Union[DictConfig, ListConfig], base_dir: str = "") -> Any:
    """
    ...
    if isinstance(config, DictConfig):
        ...
        if inherit:
            ...
            for parent_path in inherit_list:
                ...
                if base_dir and not os.path.isabs(parent_path):
