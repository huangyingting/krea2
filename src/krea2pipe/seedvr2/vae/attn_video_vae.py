...
from krea2pipe.seedvr2.parallel import get_sequence_parallel_world_size
from krea2pipe.seedvr2.logger import get_logger
from krea2pipe.seedvr2.half_precision import safe_pad_operation, safe_interpolate_operation
...
from krea2pipe.seedvr2.vae.context_parallel_lib import (
    causal_conv_gather_outputs,
    causal_conv_slice_inputs,
)
from krea2pipe.seedvr2.vae.global_config import set_norm_limit
from krea2pipe.seedvr2.vae.tiling import tiled_decode, tiled_encode
from krea2pipe.seedvr2.vae.types import (
...
class Decoder3D(nn.Module):
    ...
    def __init__(
        ...
        logger.debug("slicing_up_num: %s", slicing_up_num)
...
class VideoAutoencoderKL(diffusers.AutoencoderKL):
    ...
    def encode(self, x: torch.FloatTensor, return_dict: bool = True, tiled: bool = False,
        ...
        if tiled:
            h = tiled_encode(self, x, tile_size=tile_size, tile_overlap=tile_overlap)
        else:
    ...
    @apply_forward_hook
    def decode(
        self, z: torch.Tensor, return_dict: bool = True, tiled: bool = False,
        tile_size: Tuple[int, int] = (512, 512),
        tile_overlap: Tuple[int, int] = (64, 64),
    ) -> Union[DecoderOutput, torch.Tensor]:
        if tiled:
            decoded = tiled_decode(self, z, tile_size=tile_size, tile_overlap=tile_overlap)
        else:
...
class VideoAutoencoderKLWrapper(VideoAutoencoderKL):
    ...
    def encode(self, x: torch.FloatTensor, tiled: bool = False,
        ...
        if x.ndim == 4:
            x = x.unsqueeze(2)
        p = super().encode(x, tiled=tiled, tile_size=tile_size,
                           tile_overlap=tile_overlap).latent_dist
        # Sampling independently per tile would inject uncorrelated noise into the
        # blended overlaps, so tiled encoding always uses the distribution mode.
        z = (p.mode() if tiled else p.sample()).squeeze(2)
    ...
    def decode(self, z: torch.FloatTensor, tiled: bool = False,
        ...
        if z.ndim == 4:
            z = z.unsqueeze(2)
        x = super().decode(z, tiled=tiled, tile_size=tile_size,
