...
from krea2pipe.seedvr2.config import create_object
from krea2pipe.seedvr2.logger import get_logger, log_on_entry, log_runtime
from krea2pipe.seedvr2.diffusion import (
...
from krea2pipe.seedvr2.dit import na
...
class VideoDiffusionInfer():
    ...
    def _tiling_kwargs(self) -> dict:
        if self.vae_tiling is None:
            return {}
        size, overlap = self.vae_tiling
        return {"tiled": True, "tile_size": size, "tile_overlap": overlap}
    ...
    def configure_dit_model(self, device=None, checkpoint=None):
        ...
        if self.config.dit.get("init_with_meta_device", True):
        ...
        get_logger(__name__).info("SeedVR2 DiT parameters: %s", f"{num_params:,}")
            ...
            for sample in batches:
                ...
                with torch.autocast(device.type, vae_dtype,
                    ...
                    if use_sample:
                        ...
                        # Deterministic vae encode, only used for i2v inference
                        latent = self.vae.encode(sample, **tiling).posterior.mode().squeeze(2)
    ...
    def vae_decode(self, latents: List[Tensor]) -> List[Tensor]:
        ...
        if len(latents) > 0:
            ...
            for latent in latents:
                ...
                sample = self.vae.decode(latent, **tiling).sample
